...
import { callModel, modelAvailable, modelLabel } from './model.js';
import {
...
export function classifierMode() {
  return modelAvailable() ? modelLabel(config.models?.classifyReply) : 'regex';
}
...
async function askModel(system, userContent, labels) {
  ...
  const out = callModel({
    system,
    prompt: userContent,
    model: config.models?.classifyReply ?? null,
    jsonSchema: {
      type: 'object',
...
export async function classifyInbound(text, lead) {
  if (!modelAvailable()) return { label: regexOpener(text), via: 'regex' };
  try {
    ...
    return label
      ? { label, via: modelLabel(config.models?.classifyReply) }
...
export async function classifyAfterPitch(text) {
  if (!modelAvailable()) return { label: regexPostPitch(text), via: 'regex' };
  try {
    const label = await askModel(
      POST_SYSTEM,
      `They replied: ${JSON.stringify(text)}\n\nLabel:`,
