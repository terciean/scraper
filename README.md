...
**1. One-command setup on a new machine**

Install Node.js 22.13 or newer, then double-click `SETUP-SAAS.bat` (or run
`npm run onboard`). It installs locked app dependencies, Chromium, and the
official Codex CLI; opens the ChatGPT sign-in flow; initializes the database;
and offers to pair WhatsApp by QR. Run `npm run doctor` at any time to verify
the machine.

Qualification and reply classification use the ChatGPT account signed into
Codex on that machine. The app does not receive or store the person's ChatGPT
password, session cookie, or API key. If Codex is unavailable, enrichment still
runs and reply classification falls back to the local regex matcher.
